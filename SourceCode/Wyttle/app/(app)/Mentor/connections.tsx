  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const menteeIds = Array.from(new Set(requests.map((r) => r.mentee)));
        ...
        const threadIds = Array.from(
          new Set(requests.map((r) => r.thread_id).filter((id): id is number => id != null)),
        );
        ...
        const chatItems: MentorChatItem[] = (requests ?? []).map((r) => {
